// سرویس بررسی روزانه و ارسال نوتیفیکیشن رویدادهای ۱۰ روز آینده
function dailyEventNotificationService() {
	// فقط اگر مجوز نوتیفیکیشن داده شده باشد
	if (Notification.permission !== 'granted') return;
	// بررسی مناسبت‌های ۱۰ روز آینده
	const todayMonth = parseInt(todayFa.month);
	const todayDay = parseInt(todayFa.day);
	const eventElements = document.querySelectorAll('.event-day');
	eventElements.forEach(function(eventEl) {
		const eventText = eventEl.textContent.trim();
		const match = eventText.match(/([۰-۹0-9]{1,2})\s+(\S+)/);
		if (match) {
			let dayStr = match[1];
			let dayNum = parseInt(dayStr.convertDigits ? dayStr.convertDigits("en") : dayStr.replace(/[۰-۹]/g, function(d){return String.fromCharCode(d.charCodeAt(0)-1728);}));
			let monthName = match[2];
			let monthNum = monthLabel.indexOf(monthName) + 1;
			if (monthNum === todayMonth) {
				let diffDays = dayNum - todayDay;
				if (diffDays >= 0 && diffDays <= 10) {
					let eventTitle = '';
					let titleEl = eventEl.parentElement.querySelector('.event-title');
					if (titleEl) {
						eventTitle = titleEl.textContent.trim();
					}
					new Notification('یادآوری مناسبت', { body: eventText + (eventTitle ? ' - ' + eventTitle : '') + ' تا ' + diffDays + ' روز دیگر' });
				}
			}
		}
	});
...
// اجرای سرویس روزانه هنگام بارگذاری صفحه و هر 24 ساعت یکبار
window.addEventListener('load', function() {
	dailyEventNotificationService();
	setInterval(dailyEventNotificationService, 24 * 60 * 60 * 1000); // هر 24 ساعت
});
let notificationErrorShown = false;
// تابع ارسال نوتیفیکیشن مرورگر با پیام مناسب
function showNotification(message) {
	...
	if (Notification.permission === 'granted') {
		new Notification('یادآوری مناسبت', { body: message });
		...
		Notification.requestPermission().then(permission => {
			if (permission === 'granted') {
				...
				if (!notificationErrorShown) {
					alert('اجازه ارسال نوتیفیکیشن داده نشد. برای فعال‌سازی، تنظیمات مرورگر را بررسی کنید.');
					notificationErrorShown = true;
				}
			}
		});
...
document.addEventListener('DOMContentLoaded', function() {
	var btn = document.getElementById('testNotificationBtn');
	if (btn) {
		btn.addEventListener('click', function() {
			notifyUpcomingEvents();
		});
	}
// تابع بررسی مناسبت‌های ۱۴ روز آینده و ارسال نوتیفیکیشن مرورگر
function notifyUpcomingEvents() {
	console.log('امروز ماه:', todayFa.month, 'روز:', todayFa.day);
	const eventElements = document.querySelectorAll('.event-day');
	console.log('تعداد event-day:', eventElements.length);
	eventElements.forEach(function(eventEl) {
		console.log('event-day:', eventEl.textContent.trim());
	});
	const todayMonth = parseInt(todayFa.month);
	const todayDay = parseInt(todayFa.day);
	let found = false;
		...
		if (match) {
			...
			if (monthNum === todayMonth) {
				...
				if (diffDays >= 0 && diffDays <= 14) {
					// استخراج عنوان رویداد از عنصر مجاور با کلاس event-title
					let eventTitle = '';
					let titleEl = eventEl.parentElement.querySelector('.event-title');
					if (titleEl) {
						eventTitle = titleEl.textContent.trim();
					}
					showNotification(eventText + (eventTitle ? ' - ' + eventTitle : '') + ' تا ' + diffDays + ' روز دیگر');
					found = true;
				}
			}
		}
	...
	if (!found) {
		showNotification('مناسبتی برای ۱۴ روز آینده یافت نشد.');
	}
}
});
